	...
	public void run() {
		...
		for (int i = 0; i < 10; i++) {
			if (i%2==0) { // Les Clients pairs sont ceux qui envoient des messages (les Senders). C'est également eux qui se connectent au Serveur.
				...
				for (HashMap.Entry<Integer, MessageQueueImpl> entry : serveurqueues.entrySet()) { // Pour chacun des autres MessageQueues, on envoie le message que l'on vient de recevoir.
					if (entry.getKey() != i) {
						try {
